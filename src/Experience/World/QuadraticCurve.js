...
export default class Curve
{
    ...
    render()
    {
        ...
        for(let i=0; i + 2 < this.points.length; i += 2) {
        ...
        for(let i=0; i + 2 < this.points.length; i += 2) {
            ...
            this.drawCurve(i, true)
            this.drawCurve(i)
        }
    }

    drawCurve(i, isUpper=false) {
        ...
        if(isUpper) {
            ...
            this.context.strokeStyle = point3.color
        }        

        this.context.moveTo(point1.x, point1.y)
        this.context.quadraticCurveTo(point2.x, point2.y, point3.x, point3.y)
    ...
    getPointOfCurve(x, y) {
        ...
        for(let index = 0; index < this.points.length; index += 2) {
            for(let t=0; t <= 1; t += 0.01) {
                ...
                const pt = this.getCurve(p1, p2, p3, t);
    ...
    setPointPosition(x, y, i, reset_controls=false) {
        //Reset control points
        if(reset_controls && i % 2 == 0) {
            // Points before this one
            if(i > 0) {
                const last_point = this.points[i - 2];
                const point1 = this.getLine(last_point, {x, y} , 1/2)
                this.points[i-1].x = point1.x
                this.points[i-1].y = point1.y
            }

            if(i < this.points.length - 1) {
                const next_point = this.points[i + 2];
                const point1 = this.getLine(next_point, {x, y} , 1/2)
                this.points[i+1].x = point1.x
                this.points[i+1].y = point1.y
            }
        }
        ...
        if(this.isClosed && i === this.points.length - 1) {
            ...
            if(reset_controls) {
                const next_point = this.points[2];
                const point1 = this.getLine(next_point, {x, y} , 1/2)
    ...
    addAt(x, y, p, crossing_mode) {
        ...
        const last_point = this.points[p - 2];
        const current_point = this.points[p];

        const point1 = this.getLine({x, y}, current_point , 1/2)
        this.points[p-1].x = point1.x
        this.points[p-1].y = point1.y
        ...
        //insert new
        const point3 = this.getLine(last_point, {x, y} , 1/2)
        // const point4 = this.getLine(last_point, {x, y} , 1/3)
        this.points.splice(p - 1, 0, {x: x, y: y, isUpper: is_upper, color, ismain: true})
        this.points.splice(p - 1, 0, {x: point3.x, y: point3.y, isUpper: is_upper, color, ispotion: true})
    }
    ...
    import(json) {
        ...
        if(this.isClosed) {
            this.experience.ui.disableDrawing()
        }
    }
}